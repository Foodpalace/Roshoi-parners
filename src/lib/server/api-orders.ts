...
function mapOrder(row: OrderRow, lines: OrderLineView[], events: OrderEventView[]) {
  return {
    ...
    isCod: row.is_cod === true || (row.is_cod as unknown) === "t",
...
export const listOrders = createServerFn({ method: "GET" })
  ...
  .validator((d: { restaurantId?: string; scope?: "live" | "history" | "all" }) => d)
...
export const advanceSimulatedRider = createServerFn({ method: "POST" })
  .middleware([authMiddleware])
...
export const getOperatingSnapshot = createServerFn({ method: "GET" })
  .middleware([authMiddleware])
  .validator((d: { restaurantId?: string }) => d)
  .handler(async ({ context, data }) => {
