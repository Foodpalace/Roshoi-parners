...
export const respondToReview = createServerFn({ method: "POST" })
  .middleware([authMiddleware])
  .validator((d: { restaurantId?: string; reviewId: string; body: string }) => d)
  .handler(async ({ context, data }) => {
    ...
    return withVendor(context.userId, data.restaurantId, "notifications.view", async (sql, ctx) => {
      ...
      return {
        ...
        notifications: rows.map((n) => ({
          ...n,
          isRead: n.is_read === true || (n.is_read as unknown) === "t",
...
export const addStaff = createServerFn({ method: "POST" })
  .middleware([authMiddleware])
  .validator((d: { restaurantId?: string; email: string; role: RestaurantRole }) => d)
  .handler(async ({ context, data }) => {
    return withVendor(context.userId, data.restaurantId, "settings.staff", async (sql, ctx) => {
      if (!isRestaurantRole(data.role) || data.role === "OWNER") {
  ...
  .handler(async ({ context, data }) => {
    return withVendor(context.userId, data.restaurantId, "assistant.use", async (sql, ctx) => {
      if (!platformConfig.featureFlags.restaurant_ai) {
        return { ok: false as const, error: "NOT_CONNECTED", text: "Assistant is turned off." };
      }
      ...
      `;
      const items = await sql<{ name: string; qty: number }>`
        ...
        limit 8
      `;
      const unavailable = await sql<{ name: string; status: string }>`
      ...
      if (!apiKey) {
        const text =
          "AI is not connected in this environment. Live authorised numbers for this kitchen are shown in Analytics and Settlement. No financial figure will be invented.";
        await sql`
          insert into assistant_messages (id, restaurant_id, user_id, role, content)
          values (${newId("aim")}, ${ctx.restaurantId}, ${context.userId}, 'assistant', ${text})
        `;
        return { ok: false as const, error: "NOT_CONNECTED", text, snapshot: authorized };
      }
      ...
      if (!res.ok) {
        return { ok: false as const, error: "NOT_CONNECTED", text: `Assistant unavailable (${res.status}).` };
      }
      const body = (await res.json()) as { choices?: { message?: { content?: string } }[] };
      const text = body.choices?.[0]?.message?.content ?? "No answer.";
      await sql`
        insert into assistant_messages (id, restaurant_id, user_id, role, content)
        values (${newId("aim")}, ${ctx.restaurantId}, ${context.userId}, 'assistant', ${text})
      `;
